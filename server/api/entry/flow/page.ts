...
export default defineEventHandler(async (event) => {
  ...
  // 并行执行分页查询与统计查询；count 合并进 groupBy(_count)，减少一次全表聚合
  const [flows, sumMoney] = await Promise.all([
    pageSize == -1
      ? prisma.flow.findMany({ where, orderBy })
  ...
  sumMoney.forEach((t: any) => {
    totalFlows += Number(t._count?._all || 0);
    if (t.flowType == "收入") {
