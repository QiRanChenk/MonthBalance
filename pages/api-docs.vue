...
<template>
  ...
  <div class="p-6 max-w-6xl mx-auto text-gray-800 dark:text-gray-200">
    ...
    <div v-for="[tag, apis] in groupedPaths" :key="tag" class="mb-8">
      ...
      <div
        @click="toggleTag(tag)"
        class="flex items-center justify-between cursor-pointer bg-gray-50 hover:bg-gray-100 dark:bg-[#151a18] dark:hover:bg-gray-700 p-4 rounded-lg border border-gray-200 transition-colors"
      >
      ...
      <div v-show="expandedTags.has(tag)" class="mt-4 space-y-4">
        <div
          ...
          class="border border-gray-200 rounded-lg p-4 bg-white shadow-sm dark:bg-[#151a18]"
        >
          <div class="flex items-center gap-2">
            <span
